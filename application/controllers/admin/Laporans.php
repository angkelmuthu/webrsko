...
class Laporans extends CI_Controller 
{
	function __construct() 
	{
        ...
        $this->load->model('admin/Model_laporans');
    }

	public function index()
	{
		$data['setting'] = $this->Model_common->get_setting_data();
		$data['laporans'] = $this->Model_laporans->show();

		$this->load->view('admin/view_header',$data);
		$this->load->view('admin/view_laporans',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
				$next_id = $this->Model_laporans->get_auto_increment_id();
				foreach ($next_id as $row) {
		            $ai_id = $row['Auto_increment'];
		        }

		        $final_name = 'laporan-'.$ai_id.'.'.$ext;
		        move_uploaded_file( $path_tmp, './public/uploads/'.$final_name );

		        $final_name1 = 'laporan-banner-'.$ai_id.'.'.$ext1;
	            ...
	            $this->Model_laporans->add($form_data);

		        $success = 'News is added successfully!';
		        $this->session->set_flashdata('success',$success);
				redirect(base_url().'admin/laporans');
		    } 
		    else
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url().'admin/laporans/add');
		    }
            
        } else {
            $data['all_category'] = $this->Model_laporans->get_category();
            $this->load->view('admin/view_header',$data);
			$this->load->view('admin/view_laporans_add',$data);
			$this->load->view('admin/view_footer');
        }
	...
	public function edit($id)
	{
    	...
    	$tot = $this->Model_laporans->news_check($id);
    	if(!$tot) {
    		redirect(base_url().'admin/laporans');
        	exit;
    	}
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    if($valid == 1) 
		    {
		    	$data['news'] = $this->Model_laporans->getData($id);
		    	...
		    	if($path == '' && $path1 == '') {
		            ...
		            $this->Model_laporans->update($id,$form_data);
				}
				if($path != '' && $path1 == '') {
			...
			$data['all_category'] = $this->Model_laporans->get_category();
            $this->load->view('admin/view_header',$data);
			$this->load->view('admin/view_laporans_edit',$data);
			$this->load->view('admin/view_footer');
		}
	...
	public function delete($id) 
	{
        ...
        $this->Model_laporans->delete($id);
        $success = 'laporan is deleted successfully';
		$this->session->set_flashdata('success',$success);
		redirect(base_url().'admin/laporans');
    }
